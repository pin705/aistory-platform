<template>
  <UModal v-model:open="isOpen">
    <template #header>
      <h2 class="text-xl font-bold">{{ isEditing ? 'Chỉnh sửa Nhân vật' : 'Thêm Nhân vật mới' }}</h2>
    </template>
    <template #body>
      <div v-if="!isEditing" class="p-4 bg-gray-50 dark:bg-gray-800 rounded-md mb-6">
        <h3 class="font-semibold mb-2">Khởi tạo bằng AI</h3>
        <div class="space-y-3">
          <UFieldGroup label="Vai trò" name="ai_role">
            <USelectMenu v-model="formState.role" class="w-full" :items="['Nhân vật chính', 'Nhân vật phụ', 'Phản diện', 'Quần chúng']" />
          </UFieldGroup>
          <UFieldGroup label="Ý tưởng / Bối cảnh nhân vật" name="ai_prompt">
            <UTextarea v-model="aiPrompt" :rows="4" class="w-full" placeholder="Ví dụ: Một sát thủ về hưu, tính cách lạnh lùng nhưng yêu thương động vật. Bị cuốn vào vòng xoáy báo thù sau khi thú cưng bị hại." />
          </UFieldGroup>
          <UButton
            variant="soft"
      ...
      <UForm :state="formState" :schema="schema" @submit="save">
        ...
        <UFormField label="Vai trò" name="role" class="mb-4" v-if="isEditing">
          <USelectMenu v-model="formState.role" class="w-full" :items="['Nhân vật chính', 'Nhân vật phụ', 'Phản diện', 'Quần chúng']" />
        </UFormField>
        ...
        <UFormField label="Tiểu sử / Quá khứ" name="backstory" class="mb-4">
          <UTextarea v-model="formState.backstory" class="w-full" :rows="5" />
        </UFormField>
        <UFormField label="Năng lực / Kỹ năng" name="abilities" class="mb-4">
          <UTextarea v-model="formState.abilities" class="w-full" :rows="3" />
        </UFormField>
        <div class="flex justify-end gap-3">
          <UButton color="error" variant="ghost" @click="isOpen = false">Hủy</UButton>
          <UButton type="submit" :loading="isLoading">{{ isEditing ? 'Lưu thay đổi' : 'Thêm nhân vật' }}</UButton>
        </div>
      </UForm>
    </template>
  </UModal>

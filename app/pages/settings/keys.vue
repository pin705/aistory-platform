<template>
  <UContainer class="py-8">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold">Quản lý API Key</h1>
      <UButton icon="i-heroicons-plus" @click="isAddModalOpen = true"
        >Thêm Key mới</UButton
      >
    </div>

    <UCard>
      <UTable :data="apiKeys" :columns="columns" />
    </UCard>

    <UModal v-model:open="isAddModalOpen">
      <template #header
        ><h2 class="text-xl font-bold">Thêm API Key mới</h2></template
      >

      <template #body>
        <UForm :state="addState" :schema="addSchema" @submit="submitAddKey">
          <UFormField
          ...
          <UFormField label="API Key" name="apiKey" class="mb-4">
            <UInput
            class="w-full"
          ...
          <UButton type="submit" :loading="isLoading">Lưu Key</UButton>
        </UForm>
      </template>
    </UModal>
    ...
    <UModal v-model:open="isEditModalOpen">
      <template #header
        ><h2 class="text-xl font-bold">Chỉnh sửa API Key</h2></template
      >
      ...
      <template #body>
        ...
        >
          <UFormField
            ...
            class="mb-4"
          >
            <UInput :model-value="editState.provider" class="w-full" disabled />
          </UFormField>
          <UFormField label="API Key mới" name="apiKey" class="mb-4">
            <UInput
